...
class PatientDataHandler(DataHandler):
    ...
    def process_patient_data(self):
        ...
        self.modified_patient_ids = git_added_patient_ids.intersection(
            git_deleted_patient_ids
        )
        self.added_patient_ids = git_added_patient_ids.difference(
            git_deleted_patient_ids
        )
        self.deleted_patient_ids = git_deleted_patient_ids.difference(
            git_added_patient_ids
        )
...
class SampleDataHandler(DataHandler):
    ...
    def populate_sample_set(self):
        ...
        self.cancer_type_to_sample_count = (
            self.sample_df.groupby("CANCER_TYPE")["SAMPLE_ID"].nunique().to_dict()
        )
    ...
    def process_sample_data(self):
        ...
        self.modified_samples = {
            k: self.git_added_samples[k]
            for k in git_added_sample_ids.intersection(git_deleted_sample_ids)
        }
        self.added_samples = {
            k: self.git_added_samples[k]
            for k in git_added_sample_ids.difference(git_deleted_sample_ids)
        }
        self.deleted_samples = {
            k: self.git_deleted_samples[k]
            for k in git_deleted_sample_ids.difference(git_added_sample_ids)
        }
    ...
    def cancer_type_changed(self, sample_id):
        ...
        """
        if (
            sample_id not in self.git_added_samples
            or sample_id not in self.git_deleted_samples
        ):
            return False

        return (
            self.git_added_samples[sample_id].cancer_type
            != self.git_deleted_samples[sample_id].cancer_type
        )
    ...
    def samples_for_patient_and_cancer_type(self, patient_id, cancer_type):
        ...
        samples_for_patient_cancer_type = self.sample_df.loc[
            (self.sample_df['PATIENT_ID'] == patient_id)
            & (self.sample_df['CANCER_TYPE'] == cancer_type)
        ]
...
class Changelog:
    ...
    def get_num_modified_patients(self):
        ...
        the most recent git commit to the clinical patient file, where "modified"
    ...
    def get_num_modified_samples(self):
        ...
        the most recent git commit to the clinical sample file, where "modified"
    ...
    def aggregate_new_sample(self, sample):
        ...
        """
        self.aggregated_data[sample.cancer_type].new_samples += 1
    ...
    def aggregate_deleted_sample(self, cancer_type):
        ...
        Args:
            cancer_type (string): Type of cancer
        """
        self.aggregated_data[cancer_type].deleted_samples += 1
    ...
    def process_new_sample(self, sample):
        ...
        if sample.patient_id in self.patient_data_handler.added_patient_ids:
    ...
    def process_cancer_type_change(self, sample_id, sample):
        ...
        if sample.patient_id in self.aggregated_data[sample.cancer_type].new_patients:
            ...
            # Obtain set of samples associated with this patient + cancer type
            samples_for_cancer_type = (
                self.sample_data_handler.samples_for_patient_and_cancer_type(
                    sample.patient_id, sample.cancer_type
                )
            )
        ...
        prev_cancer_type = self.sample_data_handler.git_deleted_samples[
            sample_id
        ].cancer_type
        ...
        patient_was_deleted_from_prev_cancer_type = (
            len(
                self.sample_data_handler.samples_for_patient_and_cancer_type(
                    sample.patient_id, prev_cancer_type
                )
            )
            == 0
        )
    ...
    def aggregate_data(self):
        ...
        for sample in self.sample_data_handler.added_samples.values():
            self.process_new_sample(sample)
        ...
        for (
            ...
            self.aggregated_data[cancer_type].total_samples = sample_count
        ...
        for (
            ...
            self.aggregated_data[cancer_type].total_patients = patient_count
        ...
        for sample_id, sample in self.sample_data_handler.modified_samples.items():
            ...
            # Else this sample was modified in another way that we're not currently processing
            else:
                self.aggregated_data[sample.cancer_type].modified_samples += 1
    ...
    def write_output_data(self, output_path):
        ...
        f.write(f'Summary changelog, {todays_date}\n\n')

        f.write(
            f'Total number of patients: {self.patient_data_handler.total_patients}\n'
        )
        f.write(f'New patients: {len(self.patient_data_handler.added_patient_ids)}\n')
        f.write(
            f'Deleted patients: {len(self.patient_data_handler.deleted_patient_ids)}\n'
        )
        ...
        for cancer_type, data in sorted(self.aggregated_data.items()):
            ...
            f.write(f'\tNew samples: {data.new_samples}\n')
            ...
            f.write(f'\tDeleted samples: {data.deleted_samples}')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Generate changelog summary for new clinical patient and sample data'
    )
    ...
    if not os.path.exists(os.path.abspath(data_repo_path)):
        raise FileNotFoundError(
            f'Data repository directory not found at {data_repo_path}'
        )

    # Store absolute path to data repository
    data_repo_path = os.path.abspath(data_repo_path)

    # Ensure that data exists for this study at the given path
    if not os.path.exists(data_repo_path):
        raise FileNotFoundError(f'Study data not found at {data_repo_path}')
